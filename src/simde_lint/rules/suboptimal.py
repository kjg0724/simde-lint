...
class SuboptimalRule:
    ...
    def _grade(
        ...
        if mask.kind is ValueKind.VARIABLE:
            ...
            if definition and definition.value.kind is ValueKind.CALL_RESULT:
                source = unit.call_by_id(definition.value.call_id) if definition.value.call_id is not None else None
                if source and any(a.kind is ValueKind.LITERAL_VECTOR for a in source.args):
                    return (
                        Evidence.B,
                        f"{guard}; mask derives from a literal through {source.name}, "
